...
async def run_batch(core: ScambaiterCore) -> None:
    ...
    for index, context in enumerate(contexts, start=1):
        suggestion = core.generate_suggestion(context)
        print(f"=== Vorschlag {index}: {context.title} (ID: {context.chat_id}) ===")
        print(suggestion)
        print()
        handled = await core.maybe_interactive_console_reply(context, suggestion)
        if not handled:
            await core.maybe_send_suggestion(context, suggestion)
...
async def run() -> None:
    ...
    try:
        if not config.bot_token:
            await run_batch(core)
            return

        service = BackgroundService(core, interval_seconds=config.auto_interval_seconds)
        bot_app = create_bot_app(
            token=config.bot_token,
            service=service,
            allowed_chat_id=config.bot_allowed_chat_id,
        )
        print("BotAPI aktiv. Verfügbare Kommandos: /status /runonce /startauto /stopauto /last")
